...
const g_config = (() => {
    try {
        ...
        conf.pipe = {};
        for (const i in conf_mx.pipe) { // add new config items in accordance with the platform id
            const platformId = i.split('_').pop();
            if (platformId == conf_mx.platform.id) {
                let paramId = i.slice(0, i.length - platformId.length - 1); // gets first part of the string; '- 1' is for '_'
                conf.pipe[paramId] = conf_mx.pipe[i];
            }
        }
...
sc_server_card.StartPipeReading(g_config);
...
app.get('/', (req, res) => {
    ...
    if (req.query.content == 'data') {
        ...
        data.card = sc_server_card.GetPageData(g_config);
    ...
    HtmlBodyMain += sc_server_card.GetPage();
    HtmlBodyMain += '</table>';
...
app.get('/sound/blip_no_answer.wav', (req, res) => {
    np_fs.readFile(__dirname + '/public' + req.url, (err, file) => {
