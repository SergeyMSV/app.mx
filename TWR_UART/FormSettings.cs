...
namespace SergeM
{
    public partial class FormSettings : Form
    {
        ...
        public FormSettings()
        {
            ...
            int UARTBaudrateIndex = comboBoxUARTBaudrate.Items.IndexOf(Properties.Settings.Default.UARTBaudrate.ToString());
            comboBoxUARTBaudrate.SelectedIndex = UARTBaudrateIndex != -1 ? UARTBaudrateIndex : 0; // that means a default value
        ...
        void buttonOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (comboBoxUARTBaudrate.Text != Properties.Settings.Default.UARTBaudrate.ToString())
                {
                    UInt32 Baudrate = 0;
                    if (UInt32.TryParse(comboBoxUARTBaudrate.Text, out Baudrate))
                    {
                        Properties.Settings.Default.UARTBaudrate = Baudrate;
                    }
                    else
                    {
                        Properties.Settings.Default.UARTBaudrate = 0;
                    }
                    IsPortSettingsChanged = true;
                    UpdateSettings = true;
                }
