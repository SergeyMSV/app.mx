...
public:
	tDALLAS(boost::asio::io_context& io, const std::string& id, tSpeed speed = tSpeed::Norm)
		:tPortOneWireSync(io, id, speed)
	{}
	~tDALLAS() {}
	...
	bool WriteKey(const tID& value); // It writes ROM if only one device connected to the bus. In other words only one key must be connected to the bus and not original one.
