...
tMFRC522::tMFRC522(const share::config::port::tSPI_Config& confSPI)
...
utils::mifare::tCardType tMFRC522::GetCardType() const
{
	...
	{
	case MFRC522::PICC_Type::PICC_TYPE_MIFARE_UL:	return utils::mifare::tCardType::MIFARE_UL;
	case MFRC522::PICC_Type::PICC_TYPE_MIFARE_MINI:	return utils::mifare::tCardType::MIFARE_Mini;
	case MFRC522::PICC_Type::PICC_TYPE_MIFARE_1K:	return utils::mifare::tCardType::MIFARE_1K;
	case MFRC522::PICC_Type::PICC_TYPE_MIFARE_4K:	return utils::mifare::tCardType::MIFARE_4K;
	}
...
utils::mifare::tSector tMFRC522::GetCardClassicSector(int sectorIdx, utils::mifare::tKeyID keyID, utils::mifare::tKey key)
{
	...
	utils::mifare::tSector Sector(BlockQty, keyID);
	...
	for (int8_t i = 0; i < BlockQty; ++i)
	{
		...
		{
			Sector.SetStatus(utils::mifare::tStatus::ErrAuth);
			return Sector;
		}

		utils::mifare::tBlockCRC BlockRead{}; // Block size + CRC size = 18
		...
		if (Stat != MFRC522::StatusCode::STATUS_OK)
		{
			Sector.SetStatus(utils::mifare::tStatus::ErrRead);
			return Sector;
		}
	...
	Sector.SetStatus(utils::mifare::tStatus::OK);
	return Sector;
}

utils::mifare::tCardUL tMFRC522::GetCardUltralight()
{
	utils::mifare::tStatus Status = utils::mifare::tStatus::OK;
	utils::mifare::tCardUL Card(GetCardID());
	for (int blockIdx = 0; blockIdx < 4; ++blockIdx)
	{
		std::lock_guard<std::recursive_mutex> Lock(m_MFRC522_mtx);
		utils::mifare::tBlockCRC BlockRead{}; // Block size + CRC size = 18. Buffer size, at least 18 bytes. Also number of bytes returned if StatusCode::STATUS_OK.
		std::uint8_t ReadBytes = static_cast<std::uint8_t>(BlockRead.size());
		...
		{
			Status = utils::mifare::tStatus::ErrRead;
			break;
		}		
	}
	HaltCard();
...
bool tMFRC522::WriteCard(const utils::mifare::tCardUL& card)
{
	if (card.GetType() == utils::mifare::tCardType::MIFARE_UL)// || card.GetUID() != GetUID())
	//if (card.SAK != GetSAK() || card.UID != GetUID())
		return false;
	const utils::mifare::tLock CardLock = card.GetLock();
	...
	{
		if (!utils::mifare::tCardUL::IsPageAvailable(CardLock, pageAddr))
			continue;
...
void tMFRC522::HaltCard()
{
	...
	if (GetCardType() != utils::mifare::tCardType::MIFARE_UL)
