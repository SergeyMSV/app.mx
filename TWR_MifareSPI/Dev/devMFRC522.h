...
class tMFRC522
{
	...
	public:
...
#ifdef MXTWR_CLIENT
			:m_SPI(m_ioc, share::port::tTWREndpoint::SPI0_CS0), m_RST(m_ioc, share::port::tTWREndpoint::SPI0_CS0) // [#]
#else // MXTWR_CLIENT
			:m_SPI(id, mode, bits, speed_hz, delay_us), m_RST(id)
#endif // MXTWR_CLIENT
		{
			m_RST.SetState(true);
		}
		...
		void SetChipRST(bool state)
		{
			m_RST.SetState(state);
		}
	};
...
public:
	tMFRC522() = delete;
	explicit tMFRC522(const share::config::port::tSPI_Config& confSPI);
	...
	utils::mifare::tCardType GetCardType() const;
	std::vector<std::uint8_t> GetCardID() const;

	utils::mifare::tSector GetCardClassicSector(int sectorIdx, utils::mifare::tKeyID keyID, utils::mifare::tKey key);
	utils::mifare::tCardUL GetCardUltralight();

	bool WriteCard(const utils::mifare::tCardUL& card); // [TBD] that shall be a template for different types of cards
