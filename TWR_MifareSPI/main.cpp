...
class tCard1KBase : public utils::mifare::tCardClassic1K
{
...
public:
	...
	explicit tCard1KBase(dev::tMFRC522* cardRW) :m_pCardRW(cardRW)
	{
		...
		if (ReadCard(utils::mifare::tKeyID::A, {}, false))
			return;

		if (ReadCard(utils::mifare::tKeyID::A, utils::mifare::tKey(0xa5a4a3a2a1a0), true))
			return;
	}
...
protected:
	std::optional<utils::mifare::tSector> Card_ReadSector(std::size_t sectorIdx, utils::mifare::tKeyID keyID, utils::mifare::tKey key) override
	{
		return m_pCardRW->GetCardClassicSector(static_cast<int>(sectorIdx), keyID, key);
	}

private:
	bool ReadCard(utils::mifare::tKeyID keyID, utils::mifare::tKey key, bool cardEn)
	{
		if (cardEn)
		{
		...
		{
			std::optional<utils::mifare::tSector> Sector = ReadSector(i, keyID, key);
			if (!Sector.has_value() || Sector->GetStatus() != utils::mifare::tStatus::OK)
				break;
...
//void WriteCard(utils::mifare::tCard1K& card)
//{
...
//
//	utils::mifare::tSector Sector = CardConfig.GetSector(2);
//	//Sector.Write()
//
...
int main(int argc, char* argv[])
{
	try
	{
		...
		share::port::tGPIO PortRST(ioc, share::port::tTWREndpoint::SPI0_CS0);
#else // MXTWR_CLIENT
		tPortGPIO PortRST(DsConfig.GetSPI().ID);
#endif // MXTWR_CLIENT
		PortRST.SetState(true);

		share::config::port::tSPI_Config SPIPortCfg = DsConfig.GetSPI();
		...
		utils::mifare::tCardUL CardToWrite{}; // [TBD] - it needs mutex
		...
		while (true)
		{
			...
			{
			case utils::mifare::tCardType::MIFARE_UL:
			{
				utils::mifare::tCardUL Card = DevCardRW.GetCardUltralight();
				std::cout << Card.ToJSON() << '\n';
				break;
			}
			case utils::mifare::tCardType::MIFARE_1K:
			{
