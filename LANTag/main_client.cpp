...
static bool Contains(const asio_ip::address_v4& network, const asio_ip::address_v4& host)
{
	auto AddrNetwork = network.to_bytes();
	auto AddrHost = host.to_bytes();

	if (AddrNetwork.size() != AddrHost.size())
		return false;

	int AddrNetworkPrefix = 0; // It's not the same as Subnet Mask.
	for (int i = 0; i < AddrNetwork.size(); ++i)
	{
		if (AddrNetwork[i] != 0)
...
{
	const asio_ip::address_v4 NetworkPreffered(asio_ip::make_address_v4("192.168.10.0"));
